...
const STEP_VALUES = ['welcome', 'instructions', 'quiz', 'results'] as const;
type Step = typeof STEP_VALUES[number];

// Helper function to help TypeScript understand step comparisons
const isQuizStep = (step: Step): step is 'quiz' => {
  return step === 'quiz';
};
...
const QuizAttemptPage = () => {
  ...
  useEffect(() => {
    if (isQuizStep(step) && quizStarted) {
      const enterFullscreen = async () => {
        try {
          await requestFullscreen();
          document.addEventListener('keydown', (e) => {
  ...
  useEffect(() => {
    if (!quizStarted || !isQuizStep(step)) return;
